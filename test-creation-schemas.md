...
- ✅ `title` - назва тесту
- ✅ `lesson` - ID уроку, до якого належить тест (тести прикріплюються до уроків, а не до модулів)
...
| `lesson`       | ObjectId | ✅          | Існуючий урок                 | ID уроку (тести прикріплюються до уроків!)        |
...
### 2. Додавання тесту до уроку:

```javascript
// Оновлюємо урок, додаючи тест
const lesson = await Lesson.findById(lessonId);
lesson.tests.push(test._id);
await lesson.save();
```

**ВАЖЛИВО:** Тести прикріплюються до уроків, а не до модулів!
...
| **Обов'язкові поля** | `title`, `lesson`, `passingScore`  |
| **Автозаповнення**   | `courseId`                         |
| **Статус**           | Контролюється через API            |
| **Відстеження**      | API метаданні                      |
| **Прикріплення**     | Тести прикріплюються до уроків     |
