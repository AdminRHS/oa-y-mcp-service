  ...
  "questions": [
    {
      ...
      "image": "URL зображення (опціонально)",
      "options": [
        {
...
- 🖼️ `image` - URL зображення для тесту
...
```json
{
  ...
  "image": "https://example.com/ukraine-flag.jpg",
  "options": [
    {
      "text": "Київ",
      "isCorrect": true
...
| `image`         | String | ❌            | URL зображення                | Зображення до питання       |
...
_`options` обов'язкове для single-choice, multiple-choice, memory
...
7. **Memory тести**: Спеціальна логіка для знаходження пар
8. **Нормалізація**: Підтримка різних форматів відповідей (true/false, 1/0, Да/Нет)
9. **Зображення**: Підтримка зображень для питань
